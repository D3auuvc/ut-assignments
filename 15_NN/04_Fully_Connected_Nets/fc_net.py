...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l in range(1, len(dims)):
            ...
            self.layers.append((l,'affine_relu'))
    ...
    def loss(self, X, y=None):
        ...
        for k, layer in enumerate(self.layers[:-1], 1):
            ...
            out, out_cache1 = relu_forward(out)
            
            layer_cache.append(out_cache)
            layer_cache.append(out_cache1)
            cache[l] = layer_cache
        ...
        for k, layer in reversed(list(enumerate(self.layers[:-1], 1))):
            layer_cache = cache[k]
            dout = relu_backward(dout, layer_cache[1])
            dout, dW, db = affine_backward(dout, layer_cache[0])
            dW += self.reg * self.params[W_key(k)]
            grads.update({W_key(k): dW, b_key(k): db})
            grads.update({W_key(k): dW, b_key(k): db})
